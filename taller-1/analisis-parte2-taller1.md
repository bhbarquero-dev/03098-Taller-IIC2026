...
| `anfitrion-disponibilidad.html` *(nueva)* | Anfitrión | 7 | Gestión de disponibilidad por propiedad (línea 210): listas de períodos ocupados/bloqueados + formulario para bloquear/habilitar fechas. Se representa con listas, no con grilla de calendario, para no forzar `table`/`div` sin CSS. Se entra desde `anfitrion-propiedad-editar.html` (enlace "Gestionar disponibilidad"), no desde el listado | Media |
...
8. **❗️Pendiente (19-jul-2026):** revisar de nuevo el tema de la agenda. Hoy conviven `agenda.html` (vista general, líneas 104–106) y `anfitrion-disponibilidad.html` (gestión por propiedad, línea 210, con entrada desde `anfitrion-propiedad-editar.html`) como páginas separadas — igual que pasó con catálogo/directorio, vale la pena confirmar que la distinción es real y no una redundancia antes de dar por cerrado el diseño. Por ahora se deja así.
